...
: "${RABBITMQ_CLUSTER_NAME:=${RABBITMQ_CLUSTER_NAME:-rabbitmq}}"
: "${RABBITMQ_PEER_DISCOVERY:=${RABBITMQ_PEER_DISCOVERY:-none}}"
: "${RABBITMQ_CONSUL_HOST:=${RABBITMQ_CONSUL_HOST:-localhost}}"
: "${RABBITMQ_CONSUL_SVC:=${RABBITMQ_CONSUL_SVC:-rabbitmq}}"
: "${RABBITMQ_CONSUL_SVC_ADDR_AUTO:=${RABBITMQ_CONSUL_SVC_ADDR_AUTO:-true}}"
: "${RABBITMQ_CONSUL_SVC_ADDR_NIC:=${RABBITMQ_CONSUL_SVC_ADDR_NIC:-en0}}"
: "${RABBITMQ_CONSUL_SVC_ADDR_USE_NODENAME:=${RABBITMQ_CONSUL_SVC_ADDR_USE_NODENAME:-false}}"
: "${RABBITMQ_CONSUL_USE_LONGNAME:=${RABBITMQ_CONSUL_USE_LONGNAME:-false}}"
: "${RABBITMQ_CONSUL_SVC_TTL:=${RABBITMQ_CONSUL_SVC_TTL:-30}}"
: "${RABBITMQ_CONSUL_DEREGISTER_AFTER:=${RABBITMQ_CONSUL_DEREGISTER_AFTER:-60}}"
: "${RABBITMQ_CONSUL_INCLUDE_NODES_WITH_WARNINGS:=${RABBITMQ_CONSUL_INCLUDE_NODES_WITH_WARNINGS:-false}}"
: "${RABBITMQ_CONSUL_DOMAIN_SUFFIX:=${RABBITMQ_CONSUL_DOMAIN_SUFFIX:-example.local}}"

if [ ${RABBITMQ_PEER_DISCOVERY} == "consul" ]; then

  # Generate the configuration for consul peer_discovery_backend and add it to the rabbitmq configuration
  (
	  echo "cluster_formation.peer_discovery_backend = rabbit_peer_discovery_consul"
    echo "cluster_formation.consul.host = ${RABBITMQ_CONSUL_HOST}"
    echo "cluster_formation.consul.svc = ${RABBITMQ_CONSUL_SVC}"
    echo "cluster_formation.consul.svc_addr_auto = ${RABBITMQ_CONSUL_SVC_ADDR_AUTO}"
    echo "cluster_formation.consul.svc_addr_nic = ${RABBITMQ_CONSUL_SVC_ADDR_NIC}"
		echo "cluster_formation.consul.svc_addr_use_nodename = ${RABBITMQ_CONSUL_SVC_ADDR_USE_NODENAME}"
		echo "cluster_formation.consul.use_longname = ${RABBITMQ_CONSUL_USE_LONGNAME}"
		echo "cluster_formation.consul.svc_ttl = ${RABBITMQ_CONSUL_SVC_TTL}"
		echo "cluster_formation.consul.deregister_after = ${RABBITMQ_CONSUL_DEREGISTER_AFTER}"
		echo "cluster_formation.consul.include_nodes_with_warnings = ${RABBITMQ_CONSUL_INCLUDE_NODES_WITH_WARNINGS}"
		echo "cluster_formation.consul.domain_suffix = ${RABBITMQ_CONSUL_DOMAIN_SUFFIX}"
